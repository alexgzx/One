...
// Collect PIDs of all One-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from One install dir or wrapper cli.js
        const isAppProcess = lower.includes("one/cli.js") ||
          lower.includes("\\one\\") ||
          lower.includes("/one/") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        ...
        const isAppProcess = lowerLine.includes("one") ||
          lowerLine.includes("next-server") ||
          lowerLine.includes("cloudflared") ||
          lowerLine.includes("/bin/app/") ||
          lowerLine.includes("tray_darwin") ||
          lowerLine.includes("tray_linux");
        if (isAppProcess) {
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "One");
  }
