...
function cleanupLegacySystray({ silent = false } = {}) {
  // 1) Runtime dir: ~/.One/runtime/node_modules/systray (or %APPDATA% on Win)
  // 2) npm global nested: <npm_prefix>/node_modules/One/node_modules/systray
  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent) console.log(`[One][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent) console.warn(`[One][runtime] failed to remove ${dir}: ${e.message}`);
      }
    }
  }
}
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[One][runtime] chmod tray bin failed: ${e.message}`);
  }
}
