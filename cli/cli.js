...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.One/runtime
...
// Identifiers for killAllAppProcesses - only kill One specifically
const PROCESS_IDENTIFIERS = [
  'One'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "One")
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running One/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "one" in cmdline.
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running One/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "one".
            const cmd = line.toLowerCase();
            const isAppProcess =
...
function startServer(latestVersion) {
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "One", "db.json") : path.join(".One", "db.json"));
